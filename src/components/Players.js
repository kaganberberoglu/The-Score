...
const Players = () => {
    ...
    const shuffleData = [...data.players].sort(() => 0.5 - Math.random());
    const [filterData, setFilterData] = useState(shuffleData);
    const shuffleFilterData = [...filterData].sort(() => 0.5 - Math.random());
    const [column, setColumn] = useState("default");

    const filterFunction = (searchedThing, column) => {
        if (searchedThing.trim() === "") {
            setFilterData(shuffleData);
        } else if (column === "default" && searchedThing !== "") {
            const filteredData = shuffleData.filter((obj) =>
        ...
        } else if (searchedThing !== "" && (column === "Coin" || column === "Age" || column === "MP" || column === "Starts" || column === "Min" || column === "Gls" || column === "Ast" || column === "PK" || column === "PKatt" || column === "CrdY" || column === "CrdR")) {
            const filteredData = shuffleData.filter((obj) =>
    ...
    const sorting = (order, setOrder, column) => {
        ...
        if (order === "ASC") {
            ...
            boolean === true ?
                setFilterData(shuffleFilterData) :
                setFilterData(shuffleData);
            setOrder("ASC");
        }
    };
        ...
        <>
            {!done ? (
                <div className="flex flex-col py-10">
                    <center>
                        <p
                            className="text-lg md:text-xl lg:text-xl xl:text-3xl"
                        >
                            {changeLang ? "Yükleniyor" : "Loading"}
                        </p>
                        <ReactLoading
                            type={"cylon"}
                ...
                <div className="pb-4 flex flex-col justify-center">
                    ...
                    <div className="pb-3 px-1 flex flex-row items-end">
                        ...
                        >
                            <input
                                data-cy="premier-league-players-search-area"
                                ...
                                onChange={(e) => {
                                    e.target.value !== "" ?
                                        setBoolean(true) : setBoolean(false);
                        ...
                        >
                            <option className="font-medium" selected value="default">
                                -
                            </option>
                        ...
                        <button
                            ...
                            onClick={(e) => {
                                ...
                                teamCalc() !== 15 ?
                                    ...
                                    :
                                    navigate("/premierleague/team/4-4-2");
                            }}
                        >
                        ...
                        <tbody>
                            {filterData.map((item) => {
                                return (
                                    ...
                                    >
                                        <td data-cy="premier-league-players-info" className="px-0.5 md:px-1 lg:px-1 xl:px-1 py-0.5 font-semibold border-r border-black/25">
                                            <div
                                                ...
                                                <div className="flex flex-row items-center">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={(e) => {
                                                            changeLang ?
                                                                (theme ?
                                                                    toast.info(<div>{`${data.players[e.currentTarget.id - 1].Player} sezon içerisinde transfer oldu ve iki farklı takımda görev aldı. Bu nedenle ${findPlayer(e)} kadrosunda da ismi geçiyor.`} <FontAwesomeIcon icon={faCircleInfo} bounce /></div>, { theme: "dark" })
                                                                    :
                                                                    toast.info(<div>{`${data.players[e.currentTarget.id - 1].Player} sezon içerisinde transfer olarak iki farklı takımda görev aldı. Bu nedenle ${findPlayer(e)} kadrosunda da ismi geçiyor.`} <FontAwesomeIcon icon={faCircleInfo} bounce /></div>, { theme: "light" }))
                                                                :
                                                                (theme ?
                                                                    toast.info(<div>{`${data.players[e.currentTarget.id - 1].Player} was transferred during the season and played in two different teams. For this reason, his name is also mentioned in the ${findPlayer(e)} squad.`} <FontAwesomeIcon icon={faCircleInfo} bounce /></div>, { theme: "dark" })
                                                                    :
                                                                    toast.info(<div>{`${data.players[e.currentTarget.id - 1].Player} was transferred during the season and played in two different teams. For this reason, his name is also mentioned in the ${findPlayer(e)} squad.`} <FontAwesomeIcon icon={faCircleInfo} bounce /></div>, { theme: "light" }));
                                                        }}
                                                    >
                                                        {arr.map((element) => {
                                        ...
                                        <td className="pl-0.5 md:pl-1 lg:pl-1 xl:pl-1 py-0.5 font-semibold border-r border-black/25">
                                            <Link to="/premierleague/league/table">
                                                <img className="rounded h-5 md:h-7 lg:h-8 xl:h-9" key={item.Club} alt={item.Club} src={require(`../assets/club-logos/${item.Club === "Manchester City" ? 1 : item.Club === "Arsenal" ? 2 : item.Club === "Liverpool" ? 3 : item.Club === "Aston Villa" ? 4 : item.Club === "Tottenham" ? 5 : item.Club === "Chelsea" ? 6 : item.Club === "Newcastle Utd" ? 7 : item.Club === "Manchester Utd" ? 8 : item.Club === "West Ham" ? 9 : item.Club === "Crystal Palace" ? 10 : item.Club === "Brighton" ? 11 : item.Club === "Bournemouth" ? 12 : item.Club === "Fulham" ? 13 : item.Club === "Wolves" ? 14 : item.Club === "Everton" ? 15 : item.Club === "Brentford" ? 16 : item.Club === "Nott'ham Forest" ? 17 : item.Club === "Luton Town" ? 18 : item.Club === "Burnley" ? 19 : 20}.png`)} />
                                            </Link>
                                        </td>
