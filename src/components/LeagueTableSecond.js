...
const LeagueTableSecond = () => {
    const { theme, changeLang, selectedPlayers, dropPlayers, lineupAfterDrop, teamCalc } = useContext(AppContext);

    const navigate = useNavigate();

    const [sortLeagueTableData, setSortLeagueTableData] = useState(dataSecond.clubs);
    ...
    const sorting = (order, setOrder, column) => {
        ...
        if (order === "ASC") {
            ...
            setSortLeagueTableData(dataSecond.clubs);
            setOrder("ASC");
        }
    };
                    ...
                    {sortLeagueTableData.map((item) => {
                        return (
                            <tr
                                ...
                                key={item.Rk}
                            >
                                <td className="pl-0.5 py-0.5 font-semibold border-r border-black/25">{item.Rk}</td>
                                <td className="py-0.5 font-semibold border-r border-black/25">
                                    {item.Squad}
                                    <Link to={`/bundesliga/league/${item.Squad.split(" ").join("-").toLowerCase()}`}>
                                        <FontAwesomeIcon
                                ...
                                <td className="py-0.5 font-semibold border-r border-black/25">{item.GF}</td>
                                <td className="py-0.5 font-semibold border-r border-black/25">{item.GA}</td>
                                <td className="py-0.5 font-semibold border-r border-black/25">{item.GD}</td>
                                <td className="py-0.5 font-semibold border-r border-black/25">{item.Pts}</td>
                                <td className="py-0.5 font-semibold border-r border-black/25">{(item.Pts / item.MP).toFixed(2)}</td>
                            </tr>
                        )
                    })}
                </tbody>
...
export default LeagueTableSecond;
